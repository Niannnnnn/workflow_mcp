...
def conformation_evaluation(pred_file, true_file, cond_file, dock_mode):
    """执行构象评估计算
...
@function_tool
def conformation_evaluation_tool(pred_file, true_file, cond_file, dock_mode):
    """执行构象评估计算
    ...
    Returns:
        包含状态和结果的字典: {"status": "success/failure", "result": 评估结果或错误信息}
    """
    return conformation_evaluation(pred_file, true_file, cond_file, dock_mode)
...
def complete_molecule_workflow(pdb_file, output_dir, ref_ligand="A:330", n_samples=1, dock_mode="adgpu"):
    """执行完整的分子设计工作流：生成分子-下载分子-分子对接-下载对接结果
    ...
    Returns:
        包含状态和每个步骤结果的字典
    """
    print(f"执行完整分子设计工作流：生成分子-下载分子-分子对接-下载对接结果")
    ...
    # 设置最终结果状态
    if download_docking_result["status"] in ["success", "partial"]:
        results["status"] = "success"
        results["message"] = "完整分子设计工作流成功执行完毕"
...
complex_agent = Agent(
    ...
    7. 如果用户需要执行完整的分子设计工作流（包括分子生成、下载、分子对接和对接结果下载），请使用complete_molecule_workflow工具，这个工具可以一步完成整个流程。
    """,
    tools=[
...
async def chat(agent):
    ...
    examples = [
        ...
        "请执行完整的分子设计工作流，使用/home/zhangfn/workflow/3rfm.pdb作为受体文件生成2个分子，然后进行vina模式的分子对接，将结果保存到/home/zhangfn/workflow目录"
    ]
